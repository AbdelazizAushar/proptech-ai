...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if ((email === 'admin@aqari.sy' || email === 'omar@aqar-sy.com') && password === 'admin123') {
      localStorage.setItem('adminAuth', JSON.stringify({ name: 'مدير النظام', email }));
    ...
    const { data: admin, error: adminError } = await supabase
      ...
      .select('*')
      .eq('email', email)
      .single(); // Only check email since password_hash cannot be directly compared

    // Note: In a real app, you would use bcrypt.compare(), but for this demo:
    if (adminError || !admin) {
    ...
    // إعادة توجيه كاملة لضمان إعادة تقييم الحماية
    window.location.replace('/');
  };
          ...
          <div className="relative z-10 px-12 max-w-2xl text-white space-y-8">
            ...
            <div className="space-y-4">
              <h1 className="font-headline text-5xl font-extrabold tracking-tight leading-tight">
                أهلاً بك <br/> <span className="text-secondary-fixed">في عقارات سوريا</span>
              </h1>
          ...
          <div className="flex-grow flex items-center justify-center p-8 lg:p-16">
            <div className="w-full max-w-md space-y-10">
              ...
              <div className="space-y-2 text-right">
                ...
                <p className="text-on-surface-variant font-medium">الرجاء إدخال بيانات حسابك للمتابعة</p>
              </div>
              ...
              <div className="pt-8 border-t border-surface-container flex flex-col items-center gap-4">
                <div className="flex items-center gap-2 px-4 py-2 bg-primary/5 rounded-full">
                  <span className="material-symbols-outlined text-primary text-sm" data-icon="lock_open">lock_open</span>
                  <span className="text-sm text-on-surface-variant">يتم تشفير كافة البيانات لضمان خصوصيتك</span>
                </div>
